...
export default function TimeSlotModal({ 
  ...
  const fetchAvailableSlots = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/client/available`, {
        headers: {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setError(err.message || 'Failed to select time slot');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="   my-auto bg-white rounded-2xl z-50 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl ">
        
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Select Pickup Time</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="space-y-6">
              {slots.map((daySlots) => (
                <div key={daySlots.date} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  {/* Day Header */}
                  ...
                  <div className="p-4 grid grid-cols-2 md:grid-cols-3 gap-3">
                    {daySlots.slots.map((slot) => {
                      ...
                      const isLowAvailability = remainingSlots <= 3 && remainingSlots > 0;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 mb-2">
                            <Clock className={`w-4 h-4 ${isSelected ? 'text-orange-500' : isFull ? 'text-gray-400' : 'text-gray-500'}`} />
                            <span className={`font-semibold text-sm ${isFull ? 'text-gray-400' : 'text-gray-900'}`}>
            ...
            <div className="mb-4 p-3 bg-orange-50 rounded-lg">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-gray-900">
                  {formatTime(selectedSlot.slotStart)} - {formatTime(selectedSlot.slotEnd)}
                </span>
              </div>
            </div>
