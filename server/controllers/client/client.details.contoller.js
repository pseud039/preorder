...
const getAvailableSlots = asyncHandler(async (req, res) => {
  ...
  const formattedSlots = Object.entries(groupedSlots).map(([date, slots]) => {
    ...
    else dayLabel = slotDate.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' });
...
const createTimeSlots = asyncHandler(async (req, res) => {
  ...
  return res.status(201).json(
    new ApiResponse(201, createdSlots, `${createdSlots.length} time slots created successfully`)
  );
});
...
//   if (user?.role !== "admin") {
//     throw new ApiError(403, "Only admins can generate time slots");
//   }

  if (!startDate || !endDate || !timeSlots) {
    throw new ApiError(400, "Start date, end date, and time slots are required");
  }
  ...
  return res.status(201).json(
    new ApiResponse(201, { count: createdSlots.length }, `${createdSlots.length} time slots generated successfully`)
  );
});
export { getDetails, getAvailableSlots, updateDetails,sendOTP,resendOTP,verifyOTP,selectTimeSlot,createTimeSlots,generateTimeSlots};