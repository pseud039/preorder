...
const { name, description, address, contactNumber, imageUrl,isActive} = req.body;
if(!name){
    ...
    try {
        const newItem = await prisma.menuItem.create({
            data: {
                ...
                restaurantId: parseInt(restaurantId), // Convert to int
                isActive: true,
                isAvailable: true,
                isVeg: isVeg === 'true' || isVeg === true, // Handle string/boolean
                imageUrl: image.secure_url,
            },
        });
        
        res.status(201).json(new ApiResponse(201, "Item added successfully", newItem));
    } catch (error) {
