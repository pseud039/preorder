...
async function processEmail(email) {
  try {
    console.log(
      `[Sending] Email ${email.id} to ${email.to} (attempt ${email.attempts + 1}/${email.maxAttempts})`,
    );
    ...
    console.error(`[Failed] Email ${email.id}:`, error);
    ...
    if (attempts >= email.maxAttempts) {
      ...
      console.log(
        `[Stopped] Email ${email.id} failed permanently after ${attempts} attempts`,
      );
    } else {
      await pool.query(
        `UPDATE "EmailQueue" 
      ...
      console.log(
        `[Retry] Email ${email.id} will retry (${attempts}/${email.maxAttempts})`,
      );
    }
  }
}
